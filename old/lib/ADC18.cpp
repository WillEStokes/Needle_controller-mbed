...
float ADC18::read_ADC_18(uint8_t channel)
{
    if(ADC18_OK == adc18_write_register(ADC18_REG_GEN_CHNL_CTRL, ((uint32_t) channel << 8) | ((uint32_t) (ADC18_AIP_TEST_2MOHM_TO_AGND | ADC18_AIN_TEST_2MOHM_TO_AGND) << 16))); // set active channel
    {
        ...
        // _rdy.fall(callback(this, &ADC18::ready_interrupt));

        // _ready_flag.wait_any(EVENT_READY_FLAG);
...
void ADC18::ready_interrupt()
{
    _ready_flag.set(EVENT_READY_FLAG);
}

bool ADC18::check_ready_pin()
{
    float value = _rdy.read();
    if (value < 0.5) {
...
ADC18::ADCData_6Channel ADC18::getADCData_6Channel_continuous(uint8_t samplesToAverage)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (ADC18_OK == adc18_write_register(ADC18_REG_GEN_CHNL_CTRL, ((uint32_t)channel << 8) | ((uint32_t)(ADC18_AIP_TEST_2MOHM_TO_AGND | ADC18_AIN_TEST_2MOHM_TO_AGND) << 16))) {
            ...
            _rdy.fall(callback(this, &ADC18::ready_interrupt)); // using interrupt to detect falling edge

            // _ticker.attach_us(callback(this, &ADC18::ready_interrupt), 35); // using timed events to simulate falling edge interrupt
            ...
            _ready_flag.wait_any(EVENT_READY_FLAG);
            _rdy.fall(NULL); // detach interrupt

            // _ticker.detach(); // detach ticker
